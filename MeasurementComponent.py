...
def update_dimensions(self):
    ...
    #Unrounded mouse pos
    ur_x0,ury0 = convert_point_to_mm(self.mouse_down_pos)
    x0,y0 = convert_point_to_mm(self.mouse_current_pos)
    text = f"Position(mm): {(x0,y0)} "
    if self.is_drawing == True:
        #Unrounded mouse pos
        start_mouse_current_pos = (self.mouse_start_pos[0],self.mouse_start_pos[1])
        ur_x1,ury1 = convert_point_to_mm(start_mouse_current_pos)
        x1,y1 = convert_point_to_mm(start_mouse_current_pos)
